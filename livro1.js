...
function showImage(src) {
    return new Promise(async (resolve) => {
        ...
        setTimeout(resolve, 1500); // Espera o fade-out da imagem
    });
}
...
function nextBook() {
    ...
    fadeIn("CARREGANDO LIVRO II...", true).then(() => {
        setTimeout(() => {
            fadeOut().then(() => {
...
function renderChapterList() {
    ...
    chapterSelection.classList.remove('hidden');
    nextBookArrow.classList.remove('hidden');
    prevBookArrow.classList.add('hidden');
}
...
// Inicia com a lista de capítulos
